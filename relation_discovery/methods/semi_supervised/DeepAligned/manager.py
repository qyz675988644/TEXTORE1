...
class Manager(BaseManager):
    ...
    def __init__(self, args, data=None):
        ...
        self.train_input_ids, self.train_input_mask, self.train_pos = \
            data.input_ids, data.batch_mask, data.batch_pos
    ...
    def train(self, args, data): 
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):  
            ...
            for batch in tqdm(pseudo_train_dataloader, desc="Epoch: {} Training(All)".format(epoch+1)):
                input_ids, batch_mask, batch_pos, label_ids= self.to_cuda(*batch)
                # batch = tuple(t.to(self.device) for t in batch)
                # input_ids, input_mask, segment_ids, label_ids = batch
                loss = self.model(input_ids, batch_mask, batch_pos, label_ids, loss_fct = self.loss_fct, mode = "train")
    ...
    def get_outputs(self, args, mode, model, get_feats = False):
        ...
        total_features = torch.empty((0,args.feat_dim))
        total_logits = torch.empty((0, self.num_labels))
        ...
        for batch in tqdm(dataloader, desc="Iteration"):
            ...
            with torch.set_grad_enabled(False):
                pooled_output, logits = model(input_ids, batch_mask, batch_pos)
    ...
    def update_pseudo_labels(self, pseudo_labels, args):
        train_data = TensorDataset(self.train_input_ids, self.train_input_mask, self.train_pos, pseudo_labels)
    ...
    def eval(self, args, eval_data, is_test=True):
        ...
        if not is_test:
            ...
            y_true = eval_data.test_label_ids
            results = clustering_score(y_true, y_pred)
        ...
        reduce_dim = get_dimension_reduction(feats, args.seed)
        self.saver.features = feats
