...
from tools.utils import endless_get_next_batch, k_means, maplabels, eval_acc, flush
...
class ZeroShotModel3(nn.Module): # Pooling as relation description representation
    def __init__(self, args, config, pretrained_model, unfreeze_layers = []):
        ...
        self.bce_loss = torch.nn.BCEWithLogitsLoss()    # BCEWithLogitsLoss就是把Sigmoid-BCELoss合成一步
    ...
    def get_pretrained_feature(self, input_id, input_mask, head_span, tail_span):
        all_encoder_layers = self.pretrained_model(input_id, token_type_ids=None, attention_mask=input_mask) # (13 * [batch_size, seq_len, bert_embedding_len])
        
        encoder_layers = all_encoder_layers['hidden_states'][self.layer] # (batch_size, seq_len, bert_embedding)
    ...
    def forward(self, data, msg = 'vat', cut_gradient = False):
        if msg == 'similarity':     #降维，面向关系
            with torch.no_grad():
                input_ids, input_mask, label, head_span, tail_span = data
                input_ids, input_mask, label = list(map(lambda x:x.to(self.device),(input_ids, input_mask, label))) #lambda：参数列表-》表达式；map：根据提供的函数对指定序列做映射
            ...
            return sia_rep # (batch_size, kmeans_dim)
...
class CenterLoss(nn.Module):
    def __init__(self, dim_hidden, num_classes, lambda_c = 1.0, use_cuda = True):
        ...
        self.num_classes = num_classes      #64
    ...
    def forward(self, y, hidden):
        ...
        #transpose(0，1，2)分别表示三个维度，哪个顺序变表示维度变化
