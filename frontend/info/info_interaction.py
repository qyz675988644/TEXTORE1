...
class InfoInteraction(object):
    def __init__(self) -> None:
        ...
        super().__init__()
        self.current_path = "/home/qiuyuanzhe/TEXTORE-main"
        ...
        self.result_path = os.path.join(self.current_path, "frontend/weight_cache/results")
        self.data_path = os.path.join(self.current_path, "demo_data")
        self.caches = {}
    def exe(self, dname=None, method=None, task_type=None, kcr=None, lab_ratio=None, seed=0, istrain=1):
        ...
        eval_script = "python {} --dataname {} --method {} --task_type {}\
                    ...
                    --train_model {} --gpu_id {} --need_keyword {}".format(root, dname.lower(), method, task_type, kcr, lab_ratio, seed, istrain, 0, 1)
    ...
    def hyper_parameters(self, model_id, task_type=1, toc=False):
        ...
        if task_type == 1:
            ...
            path = os.path.join(self.config_path, "relation_discovery", "configs", modelName+".yaml")
        else:
            print("error")
    ...
    def get_format_text(self, item:dict):
        ...
        if h[0] < t[0]:
            head = text[h[0]:h[-1]]
            s = '<font color="red">[' + head[0] + ']</font>'+ '<sub> e<sub>1</sub> </sub>'
            text[h[0]] = s
            tail = text[t[0]:t[-1]]
            s1 = '<font color="blue">[' + tail[0] + ']</font>'+ '<sub> e<sub>2</sub> </sub>'
            text[t[0]] = s1
            new_text = ""
            for index ,item in enumerate(text):
                new_text = new_text + item + " "
                if index != 0 and index+1<len(text) and text[index+1] != '.' and index % 10 == 0:
                    new_text = new_text + "<br/> "
            return new_text.split(" ")
        
        else:
            head = text[t[0]:t[-1]]
            s = '<font color="blue">[' + head[0] + ']</font>' + '<sub> e<sub>2</sub> </sub>'
            text[t[0]] = s
            tail = text[h[0]:h[-1]]
            s1 = '<font color="red">[' + tail[0] + ']</font>'+ '<sub> e<sub>1</sub> </sub>'
            text[h[0]] = s1
            new_text = ""
            for index ,item in enumerate(text):
                new_text = new_text + item + " "
        ...
        # if h[0] < t[0]:
        #     sent1 = text[:h[0]]
        #     head = ['[', '<font color="red">'] + text[h[0]:h[-1]] + ['</font>', ']<sub> e<sub>1</sub> </sub>']
        #     sent_mind = text[h[-1]:t[0]]
        #     tail = ['[', '<font color="blue">'] + text[t[0]:t[-1]] + ['</font>', ']<sub> e<sub>2</sub> </sub>']
        #     sent2 = text[t[-1]:]
        #     l = 6
        #     if len(sent1) >= len(sent_mind) + len(sent2):
        #         if len(sent1) > 2*(max(len(sent_mind),len(sent2))+1) and len(sent1) > l:
        #             sent1 = sent1[:max(len(sent_mind),len(sent2))+1 ] + ['<br/>'] + sent1[max(len(sent_mind),len(sent2))+1:2*(max(len(sent_mind),len(sent2)))+1] + ['<br/>'] + sent1[2*(max(len(sent_mind),len(sent2))+1):]
        #         elif len(sent1) > l :
        #             sent1 = sent1[:max(len(sent_mind),len(sent2))+1 ] + ['<br/>'] + sent1[max(len(sent_mind),len(sent2))+1:]
        #     elif len(sent2) >= len(sent_mind) + len(sent1) :
        #         if len(sent2) >  2*(max(len(sent_mind),len(sent1))+1) and len(sent2) > l:
        #             sent2 = sent2[:max(len(sent_mind),len(sent1))+1 ] + ['<br/>'] + sent2[max(len(sent_mind),len(sent1))+1:2*(max(len(sent_mind),len(sent1)))+1] + ['<br/>'] + sent2[2*(max(len(sent_mind),len(sent1))+1):]
        #         elif len(sent2) > l :
        #             sent2 = sent2[:max(len(sent_mind),len(sent1))+1 ] + ['<br/>'] + sent2[max(len(sent_mind),len(sent1))+1:]
        #     elif len(sent_mind) >= len(sent1) + len(sent2):
        #         if len(sent_mind) > 2*(max(len(sent1),len(sent2))+1) and len(sent_mind) >l:
        #             sent_mind = sent_mind[:max(len(sent1),len(sent2))+1 ] + ['<br/>'] + sent_mind[max(len(sent1),len(sent2))+1:2*(max(len(sent1),len(sent2)))+1] + ['<br/>'] + sent_mind[2*(max(len(sent1),len(sent2))+1):]
        #         elif len(sent_mind) >l:
        #             sent_mind = sent_mind[:max(len(sent1),len(sent2))+1 ] + ['<br/>'] + sent_mind[max(len(sent1),len(sent2))+1:]
        #     if len(sent2)>2 :
        #         new_text = sent1 + head + ['<br/>'] + sent_mind + tail + ['<br/>'] + sent2
        #     else:
        #         new_text = sent1 + head + ['<br/>'] + sent_mind + tail + sent2
            
        # else:
        #     sent1 = text[:t[0]]
        #     head = ['[', '<font color=")blue">'] + text[t[0]:t[-1]] + ['</font>', ']<sub> e<sub>2</sub> </sub>']
        #     sent_mind = text[t[-1]:h[0]]
        #     tail = ['[', '<font color="red">'] + text[h[0]:h[-1]] + ['</font>', ']<sub> e<sub>1</sub> </sub>']
        #     sent2 = text[h[-1]:]
        #     l = 6
        #     if len(sent1) >= len(sent_mind) + len(sent2) :
        #         if len(sent1) > 2*(max(len(sent_mind),len(sent2)))+1 and len(sent1) > l:
    ...
    def get_pickle_data(self, log_id):
        ...
        if len(self._cond_select(run_log, log_id=int(log_id))["Local_Path"]) == 0:
            return None
        ...
        if not os.path.isfile(path):
            ...
            if len(data['labels']) == 0 or data['samples'] is None:
                dataname = self._cond_select(run_log, log_id=int(log_id))["dataset_name"].values[0]
                temp_path = os.path.join(self.data_path,'{}/test.json'.format(dataname.lower()))
                test = load_json(temp_path)
                data['samples'] = convert_examples_to_text(test)
                if len(data['labels']) == 0:
                    for item in test:
                        data['labels'].append(item['relation'])
        return data
    ...
    def get_cm_data(self, data, fine=False, task_type=1):
        ...
        if task_type == 1:
            ...
            known_list = data["known_label_list"]
            labelmap = {y:i for i, y in enumerate(known_list)}  #**:0,**:1...
...
def convert_examples_to_text(examples):
    all_data = []
