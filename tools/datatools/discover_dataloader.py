...
def load_relation_info_dict(args, tokenizer):
    ...
    with open(file_path, 'r', encoding = 'utf-8') as f:
        ...
        line = f.readline()
        while line:
            relation_name, relation_description = line.split("    ")        # 每一行以空格进行分割
            relation_description = tokenizer.tokenize(relation_description.strip())    #删除开头和结尾的空格字符
            if len(relation_description) > args.max_len - 2:    # 长度过长
                relation_description = relation_description[ : (args.max_len - 2)]  #取满足长度的那一段
            relation_description = ['[CLS]'] + relation_description + ['[SEP]']
            relation_info_dict[relation_name] = RelationInfo(len(relation_info_dict), relation_description)     #字典长度，毎进行一次循环字典长度加一
            line = f.readline()
...
class FewRelDataset(Dataset):
    ...
    def preprocess(file_path, relation_info_dict):
        ...
        data = []
        with open(file_path, 'r', encoding = 'utf-8') as f: 
            ori_data = json.load(f) #读json文件
            unique_id = 0
            for relation_name, ins_list in ori_data.items():    #关系和实体名是json文件里字典的项
                relation_id = relation_info_dict[relation_name].relation_id              
                for instance in ins_list:
                    text = clean_text(instance['tokens'])   #删掉'tokens'
